...
class SGCCircuit(tf.Module):
    ...
    def initialize_random_parameters(self, n_v1, n_lgn, n_sample):
        ...
        v1_scaled = tf.random.normal([self.n_sample, n_v1, 1, 2, 1, 1], mean = 0, stddev = 1)
    ...
    def sinkhorn(self, log_alpha, n_iters=10):
    ...
    def dlgn_scaled(self):
        if self.identify: ## apply identifiability constraint (dLGN0<=dLGN1<=dLGN2)
            ...
            for i in range(n_params):
                ...
                # get the samples/units at the ith parameter class
                p_class = self.dlgn_raw[:,:,:,i]
                
                # get the leading unit
                leading_unit = p_class[:, :, 0:1] # leading dLGN unit

                # values added to consecutive dLGN units (enforce positive vals)
                trailing_offsets = tf.nn.softplus( 
                    p_class[:, :, 1:]
                )
                ...
                # concatenate the leading unit and trailing units
                identified_units = tf.concat([leading_unit, trailing_units], axis=2)
                
                # expand dimensions to apply the matmul for permutation
                identified_units = tf.expand_dims(
                    tf.squeeze(identified_units),
                    axis = 2
                )
                print(identified_units.shape)
                # apply soft permutation
                dlgn_permutation = tf.squeeze(tf.matmul(identified_units, P[:,i]), axis = 2)

                ordered_units.append(dlgn_permutation)
