...
# ####################################################################################################################################
# ## Optimization with fts held at 0
...
# optimizer.save_state(os.path.join('c:\\', *os.getcwd().split('\\')[1:-1], 'project_datafiles', 'optimization_fts_off'))


# #####################################################################################################################################
# ## Optimization with inh held at 0 and amplitude allowed to be negative
# # set the parameter bounds
# param_bounds = {
#     "fts": [0, 175],
#     "t": [40, 125],
...
# optimizer = Optimize(model, epochs=5000, loss_threshold=0)
# optimizer.fit(X, Y_true)
# optimizer.save_state(os.path.join('c:\\', *os.getcwd().split('\\')[1:-1], 'project_datafiles', 'optimization_inh_off_3dlgn'))

# #####################################################################################################################################
# ## Optimization with fts held at 0, inh held at 0, and amplitude allowed to be negative
# # set the parameter bounds
# param_bounds = {
#     "fts": [0, 0],
#     "t": [40, 125],
...
# optimizer = Optimize(model, epochs=5000, loss_threshold=0)
# optimizer.fit(X, Y_true)
# optimizer.save_state(os.path.join('c:\\', *os.getcwd().split('\\')[1:-1], 'project_datafiles', 'optimization_inh_fts_off'))

# ###################################################################################################################################
# # Optimization with flat tuning (center at the middel range and wide bandwidth)
# # set the parameter bounds
# param_bounds = {
#     "fts": [0, 175],
#     "t": [40, 125],
...
# optimizer = Optimize(model, epochs=5000, loss_threshold=0)
# optimizer.fit(X, Y_true)
# optimizer.save_state(os.path.join('c:\\', *os.getcwd().split('\\')[1:-1], 'project_datafiles', 'optimization_flat_tuning'))
...
# with open(os.path.join('c:\\', *os.getcwd().split('\\')[1:-1], 'project_datafiles', 'permutation_test_top120_multi_sample_flat_tuning.pkl'), 'wb') as f:
#     pickle.dump(result, f)


# P = joblib.load(perm_file)

###################################################################################################################################
# Identifiable Optimization with flat tuning (center at the middel range and wide bandwidth)
# set the parameter bounds
perm_file = v1_xs_file = os.path.join(
    'c:\\', *os.getcwd().split('\\')[1:-1], 
    'project_datafiles', 
    'best_permutation.pkl'
)
P = joblib.load(perm_file)
param_bounds = {
    "fts": [0, 175],
...
model = SGCCircuit(bounds=param_bounds, enforce_permutation=P[:1000])
model.initialize_random_parameters(n_v1=2,n_lgn=3,n_sample=1000)
optimizer = Optimize(model, epochs=5000, loss_threshold=0)
optimizer.fit(X, Y_true)
optimizer.save_state(os.path.join('c:\\', *os.getcwd().split('\\')[1:-1], 'project_datafiles', 'optimization_flat_tuning_identifiable'))